...
def init_evaluation(model="base", use_backgrounds=False):
    torch.manual_seed(828)
    np.random.seed(828)
    noise_method = "noisy_linear" if model == "noisy_linear" else "none"
    ...
    agent = Agent(train_env, atoms=51, v_min=-20.0, v_max=20.0, batch_size=8, multi_step=3, discount=0.99,
                  norm_clip=10.0, lr=5e-4, adam_eps=1.5e-4, hidden_size=512, noisy_std=0.1,
                  gan_lr_mult=1e-4, perturb_factor=0.0, noise_method=noise_method, training_mode="branch")
    agent_folder = "../trained_agent/background" if use_backgrounds else \
        "../trained_agent/noisy_linear" if model == "noisy_linear" else "../trained_agent/base"
