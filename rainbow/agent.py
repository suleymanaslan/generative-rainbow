...
class Agent:
    def __init__(self, env, atoms, v_min, v_max, batch_size, multi_step, discount, norm_clip, lr, adam_eps, hidden_size,
                 noisy_std, gan_lr_mult, perturb_factor, noise_method="none", training_mode="joint", load_file=None):
    ...
    def _init_nets(self):
        ...
        if self.training_mode == "gan_feat":
            ...
            self.online_net = BranchedGeneratorDQN(self.in_channels, self.hidden_size, self.atoms, self.action_size,
                                                   ...
                                                   perturb_factor=self.perturb_factor, noise_method=self.noise_method,
                                                   residual_network=False).to(self.device)
